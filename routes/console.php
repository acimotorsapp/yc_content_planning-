...
Artisan::command('events:notify {--days=5 : Days ahead to send reminder for (default: 5)} {--date= : Specific event date to send reminder for}', function (EventNotificationService $service) {
...
Schedule::command('events:prepare-notifications')
    // ->dailyAt(config('event_notifications.prepare_time', '08:00'))
    ->everyMinute()
    ->timezone(config('event_notifications.timezone', 'Asia/Dhaka'))
    ->withoutOverlapping();

Schedule::command('events:process-notifications')
    ->everyMinute()
    ->timezone(config('event_notifications.timezone', 'Asia/Dhaka'))
    ->withoutOverlapping();
