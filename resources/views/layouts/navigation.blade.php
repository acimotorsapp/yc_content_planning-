...
<nav :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
     class="fixed inset-y-0 left-0 z-50 w-72 max-w-[85vw] bg-white border-r border-gray-200 text-gray-900 flex flex-col h-full transition-transform duration-300 ease-in-out shadow-2xl
            lg:relative lg:translate-x-0 lg:w-64 lg:max-w-none lg:shadow-xs lg:shrink-0 lg:z-40">
    <!-- Logo -->
    <div class="relative flex items-center justify-center h-16 sm:h-20 border-b border-gray-100 shrink-0 px-4">
        <a href="{{ route('dashboard') }}" class="flex items-center space-x-3 group">
            <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center text-xl font-black shadow-md shadow-blue-500/20 group-hover:scale-105 transition-transform shrink-0">
                Y
            </div>
        ...
        <button type="button"
                ...
                class="lg:hidden absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-xl text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
    </div>

    <!-- Navigation Links -->
    <div class="flex-1 overflow-y-auto overscroll-contain py-4 sm:py-6 px-3 sm:px-4 space-y-1.5">
        <a href="{{ route('dashboard') }}" @click="sidebarOpen = false" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-blue-50 text-blue-600 font-bold shadow-xs' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
        ...
        <button type="button" @click="showCreateModal = true; sidebarOpen = false" class="w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium focus:outline-none text-left cursor-pointer group">
            <svg class="w-5 h-5 shrink-0 text-gray-400 group-hover:text-gray-900 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
            <span>New Event</span>
        </button>

        @if(auth()->check() && auth()->user()->role !== 'super_admin')
        <a href="{{ route('events.my') }}" @click="sidebarOpen = false" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('events.my') ? 'bg-blue-50 text-blue-600 font-bold shadow-xs' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
            <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('events.my') ? 'text-blue-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
            <span>My Events</span>
        </a>
        @endif
        ...
        @if(auth()->check() && auth()->user()->role === 'super_admin')
            ...
            <a href="{{ route('admin.events.product') }}" @click="sidebarOpen = false" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.events.product') ? 'bg-blue-50 text-blue-600 font-bold shadow-xs' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
            ...
            <a href="{{ route('admin.events.digital') }}" @click="sidebarOpen = false" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.events.digital') ? 'bg-blue-50 text-blue-600 font-bold shadow-xs' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
            ...
            <a href="{{ route('admin.users.index') }}" @click="sidebarOpen = false" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.users.*') ? 'bg-blue-50 text-blue-600 font-bold shadow-xs' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
    ...
    <div class="border-t border-gray-100 p-4 shrink-0">
        <div class="flex items-center justify-between gap-2">
            <div class="flex items-center space-x-3 min-w-0">
                <div class="w-9 h-9 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center text-sm font-bold text-gray-700 shrink-0">
