<nav class="w-64 bg-white border-r border-gray-200 text-gray-900 flex flex-col h-screen transition-all duration-300 z-40 relative shrink-0 shadow-xs">
    ...
    <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5">
        ...
        @if(auth()->check() && auth()->user()->role === 'super_admin')
            ...
            <a href="{{ route('admin.bulk_upload.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.bulk_upload.*') ? 'bg-blue-50 text-blue-600 font-bold shadow-xs' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
                <svg class="w-5 h-5 {{ request()->routeIs('admin.bulk_upload.*') ? 'text-blue-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
