        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('click', function(e) {
                    var titleEl = e.target.closest('.js-editable-title');
                    if (!titleEl) return;
                    e.preventDefault();
                    e.stopPropagation();

                    var eventId = titleEl.getAttribute('data-event-id');
                    var current = titleEl.getAttribute('data-title') || '';
                    var textNode = titleEl.querySelector('.js-title-text');

                    Swal.fire({
                        title: 'Edit content title',
                        input: 'text',
                        inputValue: current,
                        inputPlaceholder: 'Content title',
                        showCancelButton: true,
                        ...
                        var nextTitle = result.value.trim();
                        fetch('/events/' + eventId + '/title', {
                            method: 'PATCH',
                            headers: {
                            ...
                            body: JSON.stringify({ content_title: nextTitle })
                        })
                        .then(function(response) { return response.json(); })
                        .then(function(data) {
                            if (!data.success) throw new Error(data.message || 'Could not update title');
                            if (textNode) textNode.textContent = data.content_title;
                            titleEl.setAttribute('data-title', data.content_title);
                            Swal.fire({
                                icon: 'success',
                                title: 'Title updated',
                                timer: 1400,
